...
> **Lire CE fichier en premier, et lui seul.** ~2 700 tokens — contre ~7 000 pour
> `SUIVI_PROJET.md`, et bien plus pour une ré-exploration du code.
> **Dernière MAJ : 2026-07-17 · S0 ✅ S1 ✅ S2 ✅ · S3 en cours (T3.10 ✅) — prochain : T3.5 backups**
...
CI qui mord, 4 failles fermées ; Sentry différé) ; **S3 démarré — T3.10 fait** : la livraison ne
peut plus perdre un billet. **167 tests.** Tout est sur `chantier/live-delivery-admin`,
**jamais mergé sur `main`**.

**➡️ Prochaine action : T3.5 — backups + test de restauration.** Dernier risque « on perd
quelque chose d'irrécupérable ». Puis Docker durci / compose prod. Voir `ROADMAP_REMEDIATION.md` §5.
...
| Schéma DB complet | ✅ **OUI** | migrations `0004_live_links_sent` + `0005_delivery_jobs`, réversibilité testée |
| Travail poussé | ✅ **OUI** | branche `chantier/live-delivery-admin`, 26 commits, sur `origin` — **jamais mergée sur `main`** |
| Tests | ✅ **167/167** | vérifié le 2026-07-17. 33 → 167. Tous les nouveaux sont **mutation-testés** (signature acceptant tout → 3 échecs ; idempotence retirée → 1 ; anti-rejeu retiré → 2) |
...
| Livraison billets | ✅ **outbox** | job créé dans la transaction du billet (atomique), retry 30 s→8 min ×5, dead-letter visible + alerte. Boucle 20 s, `SKIP LOCKED` (multi-worker OK). Vérifié en live : `FROM delivery_jobs` dans les logs |
| Déployable | ❌ NON | pas de compose prod, pas de Dockerfile front |

---

## ▶️ Sprint S3 — livrable (en cours)

**S2 clos** (Sentry différé — rien ne dira ce qui casse en prod). **T3.10 fait** : outbox
transactionnel, la livraison ne peut plus perdre un billet.

⚠️ **Écart assumé vs la roadmap** : T3.10 = outbox **Postgres**, pas la « file Redis » annoncée.
Le job doit naître dans la transaction du billet ; Redis ne peut pas y participer.
...
| **T3.5** | **Backups + test de restauration** | 🔴 **PROCHAIN** — aucun aujourd'hui. Un backup jamais restauré n'est pas un backup |
...
| Carte du code | `graphify-out/GRAPH_REPORT.md` — 1956 nœuds / 4534 arêtes (rebâti 2026-07-17) | ~800 tok |
...
9. **Redis est provisionné mais utilisé NULLE PART** (seulement déclaré dans `config.py`). Ne pas
   supposer qu'il porte déjà quoi que ce soit. T3.6/T3.7 seront sa première utilisation réelle.
...
|---|---|---|
| 2026-07-17 | **T3.10** — outbox transactionnel : job de livraison créé dans la transaction du billet (atomique), retry backoff ×5, dead-letter visible + alerte, boucle 20 s en `SKIP LOCKED`. `send_tickets_bg` et ses 4 appelants supprimés (dont le `create_task` fautif) — plus aucun fire-and-forget. Bugs trouvés en chemin : `email_service.aclose()` absent du lifespan, lien live en domaine dur. **167/167.** | **T3.10 clos** |
